...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickOpenAddressButton(View v) {
        // TODOx (5) Store an address in a String
        String homeAddress = "1st & Pike, Seattle";

        // TODOx (6) Use Uri.Builder with the appropriate scheme and query to form the Uri for the address
        Uri.Builder builder = new Uri.Builder();
        builder.scheme("geo")
                .path("0, 0")
                .query(homeAddress);
        Uri addressUri = builder.build();
        Log.d("Address", addressUri.toString());

        Uri anotherUri = Uri.parse("geo:0, 0?q=1600 Amphitheatre Parkway, Mountain+View, California");

        Log.d("Address 2", anotherUri.toString());

        // TODOx (7) Replace the Toast with a call to showMap, passing in the Uri from the previous step
    ...
    // TODOx (1) Create a method called showMap with a Uri as the single parameter
    private void showMap(Uri locationUri) {
        // Do steps 2 - 4 within the showMap method
        // TODOx (2) Create an Intent with action type, Intent.ACTION_VIEW
        Intent intent = new Intent(Intent.ACTION_VIEW);

        // TODOx (3) Set the data of the Intent to the Uri passed into this method
        intent.setData(locationUri);

        // TODOx (4) Verify that this Intent can be launched and then call startActivity
        if (intent.resolveActivity(getPackageManager()) != null) {
            startActivity(intent);
        }
    }
